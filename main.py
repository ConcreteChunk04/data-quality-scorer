...
def save_query_log(question, query, result_status):
    """Save query execution details to the query log file"""
    try:
        import datetime
...
def get_query_logs():
    """Get the last 10 query logs from the query log file"""
    try:
        if os.path.exists(QUERY_LOG_FILE):
            with open(QUERY_LOG_FILE, "r", encoding="utf-8") as f:
                content = f.read()
                log_entries = content.split("-" * 50 + "\n")
                return [entry.strip() for entry in log_entries if entry.strip()][-10:]
        return []
    except Exception as e:
